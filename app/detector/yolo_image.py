...
weights = glob.glob("detector/yolo/*.weights")[0]
labels = glob.glob("detector/yolo/*.txt")[0]
cfg = glob.glob("detector/yolo/*.cfg")[0]
...
layer = [layer[i - 1] for i in net.getUnconnectedOutLayers()] #i[0] -> i
...
def detect(imgpath, nn):
    ...
    # cv2.waitKey(0)
    detected_classes = list(dict.fromkeys(class_ids))
    with open('detector/temp/temp.txt', 'w') as f:
